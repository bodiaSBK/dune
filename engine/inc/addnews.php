...
HTML;
	...
	echo '<tr>
        ...
        <td class="xfields"><input type="text" style="width:350px;" name="filename" value="" /></td>
    </tr>';
...
elseif( $action == "doaddnews" ) {
	...
	$filename = $parse->process( $db->safesql( trim( $_POST['filename'] ) ) );	
	$db->query( "INSERT INTO " . PREFIX . "_post (date, autor, short_story, full_story, xfields, title, descr, keywords, category, alt_name, allow_comm, approve, allow_main, fixed, allow_rate, allow_br, votes, access, symbol, flag, tags, metatitle, filename) values ('$thistime', '{$member_id['name']}', '$short_story', '$full_story', '$filecontents', '$title', '{$metatags['description']}', '{$metatags['keywords']}', '$category_list', '$alt_name', '$allow_comm', '$approve', '$allow_main', '$news_fixed', '$allow_rating', '$allow_br', '$add_vote', '$group_regel', '$catalog_url', '1', '{$_POST['tags']}', '{$metatags['title']}', '$filename')" );
