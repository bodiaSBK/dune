...
if( $action == "list" ) {
	...
	$db->query( "SELECT id, date, title, category, autor, alt_name, comm_num, approve, fixed, news_read, votes, flag FROM " . PREFIX . "_post" . $where . " ORDER BY " . $order_by . " LIMIT $start_from,$news_per_page" );
...
<table width="100%">
    ...
    <th width=10 align="center"><input type="checkbox" name="master_box" title="{$lang['edit_selall']}" onclick="javascript:ckeck_uncheck_all()"></th>
	</tr>
...
HTML;
	...
	echo '<tr>
        <td height="29">Путь к фильму:</td>
        <td class="xfields"><input type="text" style="width:350px;" name="filename" value="' . stripslashes( $row['filename'] ) . '" /></td>
    </tr>';
	...
	if( $item_db[0] ) {
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				$filename = $db->safesql( $parse->process( trim( $_POST['filename'] ) ) );
