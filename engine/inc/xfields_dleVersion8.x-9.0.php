...
function xfieldsdatasavesql($id, $data) {
	...
	global $parse, $config, $db, $allow_br;
      ...
      foreach ($data as $xfielddataname => $xfielddatavalue) {
		...
		if( $config['allow_admin_wysiwyg'] == "yes" or $allow_br != '1' ) {
			...
			$xfielddatavalue = $db->safesql($parse->BB_Parse($parse->process($xfielddatavalue)));
...
?>
    ...
    <label for="editxfive"> <?php echo $lang['xfield_xw']; ?></label></span></div></td>
     ...
     <tr>
        <td colspan=2><div id="optional">
      <span><input type="checkbox" name="editedxfield[6]"<?php echo $checked2; ?> id="editxfiven" />
    <label for="editxfiven"> Отображать ссылкой</label></span></div></td>
    </tr>
    <tr>
