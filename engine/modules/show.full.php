...
if( ! defined( 'DATALIFEENGINE' ) ) {
	...
	if( $row ) { 
		...
		$sql_news = str_replace( 'category, descr', 'category, descr, title_eng', $sql_news );
		...
		*/
		...
		if( $config['related_news'] AND $view_template != "print" ) {
			...
			if( $buffer === FALSE ) {
				...
				if( strpos( $tpl2->copy_template, "[xfvalue_" ) !== false OR strpos( $tpl2->copy_template, "[xfgiven_" ) !== false ) { $xfound = true; $xfields = xfieldsload(); }
				else $xfound = false;
				...
				while ( $related = $db->get_row() ) {
					...
					if( ! $related['category'] ) {
						...
						$my_cat = array ();
						$my_cat_link = array ();
						$cat_list = explode( ',', $related['category'] );
