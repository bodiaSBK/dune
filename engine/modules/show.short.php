...
if( $allow_active_news ) {
	...
	// TPLPREFIX
	if ( empty( $_tplprefix ) and ! $_xshortmode ) {
		if ( $_COOKIE['dle_shortstory'] == 1 ) $_tplprefix = '1';
		else if ( $_COOKIE['dle_shortstory'] == 2 ) $_tplprefix = '2';
		else $_tplprefix = '';
	}
	
	if( isset( $view_template ) and $view_template == "rss" ) {
	} elseif( $category_id and $cat_info[$category_id]['short_tpl'] != '' ) $tpl->load_template( $cat_info[$category_id]['short_tpl'] . '.tpl' );
	elseif( $do == 'search_xf' && $config['sf_show_theme'] ) {
		if ( $config['sf_show_theme'] == 'shortstory' ) {
			$tpl->load_template( 'shortstory' . $_tplprefix . '.tpl' );
		} else $tpl->load_template( $config['sf_show_theme'] . '.tpl' );
	} else $tpl->load_template( 'shortstory' . $_tplprefix . '.tpl' );
	...
	$sql_select = str_replace( 'category, alt_name', 'category, alt_name, title_eng', $sql_select );
	...
	while ( $row = $db->get_row( $sql_result ) ) {
		...
		$tpl->set( '', $news_find );
		$tpl->set( '{title_eng}', stripslashes( $row['title_eng'] ) );
		...
		$tpl->set( '[rx_file]', "" );
		$tpl->set( '[/rx_file]', "" );
		$tpl->set_block( "'\\[not_rx_file\\](.*?)\\[/not_rx_file\\]'si", "" );

		} else {
			$tpl->set_block( "'\\[rx_file\\](.*?)\\[/rx_file\\]'si", "" );
			$tpl->set( '[not_rx_file]', "" );
			$tpl->set( '[/not_rx_file]', "" );
		}
		$tpl->compile( 'content' );
	...
	//####################################################################################################################
	if( ! isset( $view_template ) AND $count_all AND $config['news_navigation'] ) {
	...
	if($do == 'search_xf')
		...
		$pages .= "<a href=\"" . $url_page . "/page/" . $j . "/\">$j</a> ";
					...
					if( $cstart != $enpages_count ) {
						...
						if( $config['allow_alt_url'] == "yes" ) {
							if($do == 'search_xf') $pages .= $nav_prefix . "<a href=\"" . $url_page . "page:{$enpages_count}/\">{$enpages_count}</a>";
							else $pages .= $nav_prefix . "<a href=\"" . $url_page . "/page/{$enpages_count}/\">{$enpages_count}</a>";
